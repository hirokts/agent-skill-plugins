...
allowed-tools: Read Bash(bash ~/.codex/skills/peer/scripts/peer.sh:*)
---

# Peer

`peer.sh` と `messages.md` を介して他のAIエージェントと非同期でメッセージを交換する。
...
```bash
bash ~/.codex/skills/peer/scripts/peer.sh <send|read|clear> [message]
```
